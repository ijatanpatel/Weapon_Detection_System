...
# Email cooldown period in seconds (2 minutes)
EMAIL_COOLDOWN = 120
...
def can_send_email():
    if settings['last_email_time'] is None:
        return True
    
    current_time = datetime.now()
    time_diff = (current_time - settings['last_email_time']).total_seconds()
...
def send_alert(detection_type, confidence, detections_count):
    ...
    try:
        ...
        with smtplib.SMTP('smtp.gmail.com', 587) as server:
            ...
            # Only update last_email_time if email was sent successfully
